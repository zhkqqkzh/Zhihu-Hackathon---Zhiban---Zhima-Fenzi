...
// is_concept 是非概念拦截第三层（前端字段已预留）：非概念时早退，不要编解释。
function buildPrompt(term, context) {
  return '知乎回答片段：' + context + '\n' +
    '用户选中「' + term + '」。直接输出 JSON（不要思考过程、不要 markdown 代码块）：\n' +
    '{"is_concept":true,"definition":"一句话定义，≤40字","context_why":"它在这段话里的作用，作者为何提到，≤60字，紧扣片段","prerequisites":["最多2个"]}\n' +
    '若选中内容不是值得解释的概念（虚词、标点、人名、机构名、无意义片段），只返回 {"is_concept":false}，不要编解释。\n' +
